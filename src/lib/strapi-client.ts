...
export class StrapiClient {
  ...
  constructor(options: StrapiClientOptions) {
    ...
    this.httpClient = new HttpClient(options.url, options.apiToken);
  ...
  private _initStrapiAuthClient(clientInstance: HttpClient) {
    return new StrapiAuthClient(clientInstance, this.options);
  }
}
